...
func TestGetUserId(t *testing.T) {
	...
	re1.Header.Add("x-user-id", "test-user-id-1")
	re2, err := http.NewRequestWithContext(context.Background(), http.MethodGet, "/", bufio.NewReader(bytes.NewReader([]byte("t"))))
	if err != nil {
		t.Error(err)
	}
	re2.Header.Add("x-user-id", "test-user-id-2")
	re3, err := http.NewRequestWithContext(context.Background(), http.MethodGet, "/", bufio.NewReader(bytes.NewReader([]byte("t"))))
	if err != nil {
		t.Error(err)
	}
	tests := []struct {
		...
		{name: "GetUserId-1", args: args{req: re1}, want: func() string { return re1.Header.Get("x-user-id") }},
		{name: "GetUserId-2", args: args{req: re2}, want: func() string { return re2.Header.Get("x-user-id") }},
		{name: "GetUserId-3", args: args{req: re3}, want: func() string { return re3.Header.Get("x-user-id") }},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var v string
			got := GetUserId(tt.args.req)
			if got != nil {
				v = *got
			}
			if v != tt.want() {
				t.Errorf("GetUserId() = %v, want %v", v, tt.want())
			}
		})
	}
}