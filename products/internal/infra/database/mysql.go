...
func (m *mySQL) WithProductDatabase() services.Database {
	m.database = productDatabase
	return m
}
...
func (m *mySQL) Init(ctx context.Context) error {
	...
	case productDatabase:
		databaseMasterDsn = m.ServiceManager().Environment().ProductDatabaseDsn()
		databaseReplicaDsn = m.ServiceManager().Environment().ProductReplicaDatabaseDsn()
	default:
